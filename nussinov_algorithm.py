...
def stack_based_backtrace(dp, sequence, pairs):
    ...
    n = len(sequence)
    structure = ["." for _ in range(n)]  # fill secondary structure as unpaired
    stack = [(0, n - 1)]  # start at top-right corner of the DP table
    visited_cells = []  # List to track visited cells
    ...
    while stack:
        ...
        if dp[i + 1][j] == dp[i][j]: 
            ...
            stack.append((i, j - 1))
        elif dp[i + 1][j - 1] + 1 == dp[i][j] and pairs.get(sequence[i]) == sequence[j]: 
